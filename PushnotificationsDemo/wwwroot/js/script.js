...
document.addEventListener('DOMContentLoaded', function (event) {
    ...
    const pushBtn3 = document.getElementById('btnSendMessage');
    if (!(navigator.serviceWorker && 'PushManager' in window)) {
        // service worker is not supported, so it won't work!
    ...
    registerServiceWorker().then(function () {
        ...
        pushBtn3.addEventListener('click', function (event) {
           
            event.preventDefault();        
            sendMessage(document.getElementById("title").value, document.getElementById("message").value, 10);
        });
        updateUnsubscribeButtons();
    });
